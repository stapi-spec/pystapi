...
- Pagination on `GET /searches/opportunities/{searchRecordId}/statuses`, which previously published neither `next` nor `limit` because its backend had nothing to paginate.
...
- **BREAKING** `GetOpportunitySearchRecordStatuses` gained `next` and `limit` parameters and returns `Maybe[Page[OpportunitySearchStatus]]` in place of `Maybe[list[OpportunitySearchStatus]]`. The endpoint answers with an `OpportunitySearchStatusCollection` rather than a bare JSON array, so it can carry links and a total like every other collection.
