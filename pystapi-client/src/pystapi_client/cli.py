...
def collections(
    ...
    *,
    ...
    try:
        ...
        raise STACError(
            f"The client at {client.self_href} is OK, but one or more of the collections is invalid."
        ) from e
    return 0
...
def set_conforms_to(client: Client, clear: bool, remove: list[str] | None, add: list[str] | None) -> None:
...
def parse_args(args: list[str]) -> dict[str, Any]:
    ...
    collections_group.add_argument("--bbox", help="Bounding box (min lon, min lat, max lon, max lat)", nargs=4)
    ...
    collections_group.add_argument("--fields", help="Control what fields get returned", nargs="*")
    ...
    output_group.add_argument("--matched", help="Print number matched", default=False, action="store_true")
        ...
        if os.path.exists(parsed_args["intersects"]):
            with open(parsed_args["intersects"]) as f:
                ...
                if data["type"] == "Feature":
                    ...
                    logger.warning(
                        "When the input to intersects is a FeatureCollection, only the first feature geometry is used."
                    )
