...
DatetimeOrTimestamp = Optional[datetime_ | str]
Datetime = str
DatetimeLike = Union[
    DatetimeOrTimestamp,
...
class BaseSearch(ABC):
    ...
    @staticmethod
    def _format_filter_lang(_filter: FilterLike | None, value: FilterLangLike | None) -> str | None:
        if _filter is None:
            return None
    ...
    def _format_datetime(self, value: DatetimeLike | None) -> Datetime | None:
        ...
        if not components:
            ...
            raise Exception(f"too many datetime components (max=2, actual={len(components)}): {value}")
...
class ItemSearch(BaseSearch):
    ...
    def matched(self) -> int | None:
        ...
        resp = self._stac_io._read_json(self.url, method=self.method, parameters=params)
        found = None
        if "context" in resp:
