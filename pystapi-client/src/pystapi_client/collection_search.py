...
TemporalInterval = tuple[datetime | None, datetime | None]
...
def _extent_matches(
    ...
    bbox_overlaps = not bbox or (
        any(bboxes_overlap(bbox, tuple(collection_bbox)) for collection_bbox in extent.spatial.bboxes)
    )
    ...
    search_temporal_interval = temporal_interval_str.split("/") if temporal_interval_str else []

    # replace .. in open intervals with actual strings
    if search_temporal_interval:
        if search_temporal_interval[0] == "..":
            search_temporal_interval[0] = datetime.min.replace(tzinfo=timezone.utc).isoformat()
        if search_temporal_interval[1] == "..":
            search_temporal_interval[1] = datetime.max.replace(tzinfo=timezone.utc).isoformat()

    datetime_overlaps = not temporal_interval_str or (
        any(
            temporal_intervals_overlap(
                (
                    datetime.fromisoformat(search_temporal_interval[0].replace("Z", "+00:00")),
                    datetime.fromisoformat(search_temporal_interval[1].replace("Z", "+00:00")),
                ),
                (
...
def collection_matches(
    ...
    try:
        ...
        bbox_overlaps, datetime_overlaps = _extent_matches(extent, bbox, temporal_interval_str)

    # check for overlap between the provided free-text search query (q) and the
    # collection's title, description, and keywords
    text_fields: dict[str, str] = {
        key: text for key, text in collection_dict.items() if text and key in ["title", "description", "keywords"]
    }
...
class CollectionSearch(BaseSearch):
    ...
    def __init__(
        ...
        *,
        ...
        if client and client._stac_io is not None and stac_io is None:
            ...
            self._collection_search_extension_enabled = client.conforms_to(ConformanceClasses.COLLECTION_SEARCH)
            ...
            self._collection_search_extension_enabled = collection_search_extension_enabled
            self._collection_search_free_text_enabled = collection_search_free_text_enabled
            self._validate_client_side_args()
    ...
    def matched(self) -> int | None:
        ...
        if self._collection_search_extension_enabled and self._collection_search_free_text_enabled:
            if "context" in page:
    ...
    def collections(self) -> Iterator[Collection]:
        ...
        for collection in self.collections_as_dicts():
            ...
            yield Collection.from_dict(collection, root=self.client, preserve_dict=False)
            ...
            for page in self._stac_io.get_pages(self.url, self.method, self.get_parameters()):
                ...
                if collections:
                    ...
                    if self._max_items and num_collections > self._max_items:
                        ...
                        page["collections"] = collections[0 : -(num_collections - self._max_items)]
                    else:
    ...
    def collection_list(self) -> list[Collection]:
        ...
        # already signed in collections_as_dicts
        return [
            Collection.from_dict(collection, preserve_dict=False, root=self.client) for collection in collection_list
        ]
